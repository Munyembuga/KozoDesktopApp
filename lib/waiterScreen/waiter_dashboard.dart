...
class WaiterStats {
  ...
  factory WaiterStats.fromJson(Map<String, dynamic> json) {
    return WaiterStats(
...
class DashboardStats {
  ...
  factory DashboardStats.fromJson(Map<String, dynamic> json) {
    return DashboardStats(
      ...
      waiterStats: json['waiter_stats'] != null
          ? WaiterStats.fromJson(json['waiter_stats'])
          : null,
    );
  }
}
...
class WaiterDashboardScreen extends StatefulWidget {
  ...
  @override
  State<WaiterDashboardScreen> createState() => _WaiterDashboardState();
}

class _WaiterDashboardState extends State<WaiterDashboardScreen> {
  final Dio _dio = Dio();
  ...
  Future<void> _fetchDashboardStats() async {
    try {
      ...
      // Get current user to get waiter ID
      ...
      final response = await _dio.post(
        '${AppConfig.baseUrl}/Orders/Waiter_dashboard_stats',
        data: {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Restaurant Dashboard v.1.6 $_role',
              ...
              : SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // Waiter Statistics
                      if (_stats!.waiterStats != null) ...[
                        const SizedBox(height: 24),
                        _buildWaiterStats(_stats!.waiterStats!),
                      ],
                    ],
                  ),
                ),
  ...
  Widget _buildWaiterStats(WaiterStats waiterStats) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
        ...
        children: [
          Row(
            children: [
              ...
              const Text('Waiter Statistics', style: AppTextStyles.heading),
            ],
          ),
          const SizedBox(height: 16),
          ...
          Row(
            children: [
              Expanded(
                child: _buildWaiterMetricCard(
                  'Total Transactions',
                  '${waiterStats.totalTransactions}',
                  Icons.receipt,
                  Colors.blue,
                ),
              ),
              ...
              Expanded(
                child: _buildWaiterMetricCard(
                  'Total Revenue',
                  waiterStats.formattedTotalRevenue,
                  Icons.attach_money,
                  Colors.green,
                ),
              ),
          ...
          if (waiterStats.revenueByMethod.isNotEmpty) ...[
            const Text(
