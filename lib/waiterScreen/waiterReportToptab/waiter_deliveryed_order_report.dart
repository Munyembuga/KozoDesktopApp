...
class WaiterDeliveredOrderReport extends StatefulWidget {
  const WaiterDeliveredOrderReport({Key? key}) : super(key: key);

  @override
  State<WaiterDeliveredOrderReport> createState() =>
      _WaiterDeliveredOrderReportState();
}

class _WaiterDeliveredOrderReportState
    extends State<WaiterDeliveredOrderReport> {
  ...
  Future<void> _fetchUnpaidOrders() async {
    ...
    try {
      ...
      final orders = await _waiterReportingService.getUnpaidOrders(
  ...
  List<dynamic> get _paginatedOrders {
    if (_unpaidOrders == null || _unpaidOrders!.isEmpty) {
      return [];
    }

    final startIndex = (_currentPage - 1) * _pageSize;
    final endIndex = startIndex + _pageSize > _unpaidOrders!.length
        ? _unpaidOrders!.length
        : startIndex + _pageSize;

    if (startIndex >= _unpaidOrders!.length) {
      return [];
    }

    return _unpaidOrders!.sublist(startIndex, endIndex);
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  if (_unpaidOrders != null)
                    Text(
                      'Total Unpaid Orders: ${_unpaidOrders!.length}',
                      style: TextStyle(
                        fontWeight: FontWeight.bold,
                        color: AppColors.primary,
                      ),
            ...
            else if (_unpaidOrders == null || _unpaidOrders!.isEmpty)
              const Center(
                child: Padding(
                  padding: EdgeInsets.all(32.0),
                  child: Column(
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    // Page information
                    Text(
                      'Showing ${_paginatedOrders.length} of ${_unpaidOrders!.length} orders | Page $_currentPage of $_totalPages',
                      style: TextStyle(
                        color: AppColors.grey,
                      ),
                    ),
