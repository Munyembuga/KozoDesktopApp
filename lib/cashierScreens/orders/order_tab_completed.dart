...
// Define PaymentMethod class for this file

class OrderTabCompleted extends StatefulWidget {
  const OrderTabCompleted({super.key});

  @override
  State<OrderTabCompleted> createState() => _OrderTabCompletedState();
}

class _OrderTabCompletedState extends State<OrderTabCompleted> {
  ...
  Future<void> _fetchPaymentMethods() async {
    try {
      final methods = await OrderService.fetchPaymentMethods();
      if (mounted) {
        setState(() {
  ...
  Future<void> _fetchCompletedOrders() async {
    try {
      ...
      final orders = await OrderService.fetchCompletedOrders(
  ...
  Widget _buildOrderCard(Order order, BuildContext context) {
    ...
    return Container(
        ...
        padding: const EdgeInsets.symmetric(
            horizontal: 20, vertical: 16), // 👈 left & right padding
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color:
                isSelected ? AppColors.primary : Colors.blue.withOpacity(0.3),
