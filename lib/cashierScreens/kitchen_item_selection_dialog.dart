...
import 'served_order_detail.dart';

class KitchenItemSelectionDialog extends StatefulWidget {
  final KitchenItemsResponse response;
  final Function(List<KitchenItem>) onItemsSelected;

  const KitchenItemSelectionDialog({
    Key? key,
  ...
  @override
  State<KitchenItemSelectionDialog> createState() =>
      _KitchenItemSelectionDialogState();
}

class _KitchenItemSelectionDialogState
    extends State<KitchenItemSelectionDialog> {
  ...
  String _getUniqueItemKey(KitchenItem item) {
    ...
    // Using item.id (order_item_id) which is unique within the order instead of menu_item_id
    return '${item.id}_${item.specificationId}_${item.quantity}_${item.totalPrice}_${item.comment ?? "no_comment"}';
  }
        ...
        children: [
          ...
          if (item.hasComment)
            Container(
              ...
              child: Row(
                children: [
                  Icon(Icons.comment, size: 12, color: Colors.orange[600]),
