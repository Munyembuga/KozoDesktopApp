  ...
  Future<void> _addItemsToOrder(List<CartItem> cartItems) async {
    try {
      ...
      final newItems = cartItems.map((cartItem) {
        // Extract item ID and specification ID from cart item
        // Note: You may need to store specificationId separately in CartItem model
        // For now, assuming the cart item has the necessary data
        return {
          'itemId': cartItem.id,
          'specificationId': cartItem.specificationId ??
              cartItem.id, // You may need to add this field to CartItem
          'quantity': cartItem.quantity,
          'price': cartItem.price,
          'total': cartItem.price * cartItem.quantity,
        };
      }).toList();
      print('New Items to Add: $newItems'); // Debug log
      // Call API to add items to existing order
      final result = await OrderService.addItemToOrder(
        orderId: _orderDetail!.id,
        items: newItems,
      );
      print('Add Items Result: $result'); // Debug log
      // Hide loading
      Navigator.of(context).pop();
