...
class WaiterOrderServices {
  ...
  static Future<List<Order>> fetchPendingOrders({
    ...
    try {
      ...
      final response = await _dio.post(
        '$baseUrl/Orders/waiter_pending_order',
  ...
  static Future<OrderDetail> fetchOrderDetail(int orderId) async {
    try {
      final response = await _dio.post(
        '$baseUrl/Orders/waiter_pending_detailed',
        data: {
  ...
  static Future<List<Order>> fetchCompletedOrders({
    ...
    try {
      ...
      final response = await _dio.post(
        '$baseUrl/Orders/waiter_completed_order',
        data: requestData,
      );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          throw Exception(
              'Failed to process order: ${data['message'] ?? 'Unknown error'}');
        }
      } else {
        throw Exception('Failed to process order: HTTP ${response.statusCode}');
      }
    } on DioException catch (e) {
      if (e.type == DioExceptionType.connectionTimeout ||
          e.type == DioExceptionType.receiveTimeout) {
  ...
  static Future<Map<String, dynamic>> addItemToOrder({
    ...
    try {
      ...
      print('🔍 Adding items to order with request: $requestData');
      ...
      print('📦 Add items response status: ${response.statusCode}');
      print('📦 Add items response data: ${response.data}');
  ...
  static Future<bool> cancelOrder(int orderId) async {
    try {
      final response = await _dio.post(
        '$baseUrl/Orders/waiter_cancel_order',
        data: {
          'orderId': orderId.toString(),
        },
      );
